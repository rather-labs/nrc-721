...
// FIXED_LEN + Factory::name(usize) + Factory::symbol(usize) + Factory::base_token_uri(usize) size
...
/// 3) base_token_uri: <size: u16> + <content>
/// None of [name, symbol, base_token_uri] are mandatory, but 0usize must be placed as length in that case
#[derive(Debug, Clone)]
pub struct Factory {
    pub name: Vec<u8>,
    pub symbol: Vec<u8>,
    pub base_token_uri: Vec<u8>,
}
...
impl Factory {
    pub fn from_data(raw_data: &[u8]) -> Result<Self, Error> {
        ...
        // DYN_MIN_LEN * 2: the min length of symbol + the min length of base_token_uri
        ...
        // DYN_MIN_LEN: the min length of base_token_uri
        if raw_data.len() < symbol_index + symbol_len + DYN_MIN_LEN {
            return Err(Error::FactoryDataInvalid);
        }
        let symbol = raw_data[symbol_index..(symbol_index + symbol_len)].to_vec();

        // base_token_uri reading
        let base_token_uri_index = FIXED_LEN + name_len + symbol_len;
        let base_token_uri_len =
            parse_dyn_vec_len(&raw_data[base_token_uri_index..(base_token_uri_index + DYN_MIN_LEN)]);
        let base_token_uri = raw_data[base_token_uri_index..(base_token_uri_index + base_token_uri_len)].to_vec();
    ...
    pub fn immutable_equal(&self, other: &Factory) -> bool {
        self.name == other.name && self.symbol == other.symbol && self.base_token_uri == other.base_token_uri
    }
}
